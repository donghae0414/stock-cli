...
func TestBuildCashCancelRequestDefaultsQuantity(t *testing.T) {
	req, err := buildCashCancelRequest(cashCancelOptions{
		StockCode:       "005930",
...
func TestBuildCashCancelRequestRejectsUnexpectedArgs(t *testing.T) {
	_, err := buildCashCancelRequest(cashCancelOptions{}, []string{"extra"})
	require.Error(t, err)
...
func withCashCancelStockCode(opts cashCancelOptions, stockCode string) cashCancelOptions {
	opts.StockCode = stockCode
	return opts
}

func withCashCancelOriginalOrderID(opts cashCancelOptions, originalOrderID string) cashCancelOptions {
	opts.OriginalOrderID = originalOrderID
	return opts
}

func withCashCancelQuantity(opts cashCancelOptions, quantity string) cashCancelOptions {
	opts.Quantity = quantity
	return opts
}

func withCashCancelVenue(opts cashCancelOptions, tradingVenue string) cashCancelOptions {
